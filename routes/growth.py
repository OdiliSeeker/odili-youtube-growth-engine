...
async def mark_schedule_posted(
    ...
    # Close the loop: land the announcement draft in the email queue so the
    # admin can approve + send it from the Email Queue tab (spec PART 2).
    from app.services import email_queue_service
    queued = email_queue_service.create_draft(
        db,
        subject=email.get("subject", row.title),
        body="\n".join(p for p in [email.get("hook", ""), email.get("body", "")] if p),
        source="video_posted",
        video_url=email.get("youtube_url") or None,
        dedup=False,
    )
    ...
    return {
        ...
        "schedule_id": row.id,
        "email": email,
        "queued_email_id": queued.id if queued else None,
    }
