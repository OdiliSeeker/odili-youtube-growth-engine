...
@router.post("/newsletter/draft", tags=["Newsletter"])
async def draft_newsletter(
    db: Session = Depends(get_db),
    _: None = Depends(verify_admin),
) -> dict:
    """
    Generate an AI weekly newsletter and queue it as a DRAFT for admin approval
    (instead of sending immediately). Reuses the EmailQueue draft→approve→send
    pipeline — the admin reviews it in the Email Queue tab before anything ships.
    """
    ...
    try:
        ...
        logger.error("Newsletter draft AI generation failed: %s", exc)
        _handle_ai_error(exc)
    except ValueError as exc:
        logger.error("Newsletter draft content parse failed: %s", exc)
        raise HTTPException(status_code=502, detail=f"Content generation failed: {exc}") from exc

    subject = content["subject"]
    body = render_newsletter_text(content)
    # Conversion engine: every newsletter draft ends with a reply prompt
    # (engagement driver) unless the AI already included one.
    if "hit reply" not in body.lower() and "reply to this email" not in body.lower():
        body += (
            "\n\nP.S. Hit reply and tell me — what's the one question about this "
            "you've never gotten a straight answer to? I read every response."
        )
    item = email_queue_service.create_draft(db, subject=subject, body=body, source="newsletter")
    if item is None:
        raise HTTPException(status_code=500, detail="Failed to queue newsletter draft.")
    return {
        "status": "drafted",
        "message": "Newsletter drafted and queued for approval.",
        "id": item.id,
        "subject": subject,
    }
