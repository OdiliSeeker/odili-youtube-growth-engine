...
async def generate_idea_bundle(topic: str) -> dict:
    ...
    Enriched fail-silently with Growth Brain optimized titles + viral hooks so the
    Traffic Engine surfaces higher-CTR options; enrichment never blocks the bundle.
    """
    ...
    await _attach_growth_brain(bundle, topic)
    return bundle


async def _attach_growth_brain(bundle: dict, topic: str) -> None:
    """Add Growth Brain optimized_titles + viral_hooks (fail-silent, additive)."""
    try:
        from app.services import title_scorer, viral_hook_engine

        opt = await title_scorer.generate_optimized_titles(topic)
        bundle["optimized_titles"] = opt.get("top_titles", [])
        hooks = await viral_hook_engine.generate_hooks(topic)
        bundle["viral_hooks"] = hooks.get("short_hooks", [])[:5]
    except Exception as exc:  # noqa: BLE001 — enrichment must never break the bundle
        logger.info("Traffic Engine Growth Brain enrichment skipped (%s)", exc)
