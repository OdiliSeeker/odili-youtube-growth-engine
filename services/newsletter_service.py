...
async def generate_newsletter_content() -> dict:
    ...
    try:
        ...
        _attach_subject_alternatives(data)
        return data
    except (json.JSONDecodeError, KeyError) as exc:
        raise ValueError(f"AI returned an unexpected format: {exc}. Raw: {raw[:400]}") from exc


def _attach_subject_alternatives(data: dict) -> None:
    """Add high-CTR subject-line alternatives from the trigger library (fail-silent).

    Pure-deterministic (no AI / no quota) — additive key, never breaks rendering.
    """
    try:
        from app.services import click_trigger_library

        subject = str(data.get("subject") or "").strip()
        if subject:
            data["subject_alternatives"] = click_trigger_library.top_phrases(subject, per_category=1)
    except Exception:  # noqa: BLE001 — enrichment must never break the newsletter
        pass
