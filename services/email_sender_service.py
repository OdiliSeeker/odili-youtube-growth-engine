...
def send_admin_notice(subject: str, text_body: str) -> SendResult:
    """
    Send an operational notice to the ministry/admin address (the verified
    SENDGRID_FROM_EMAIL — a real inbox, unlike the no-reply subscriber flow).

    For internal alerts ONLY (e.g. a lead-discovery quota stall). This never
    touches the subscriber list or the EmailQueue, so it can never leak an
    operational message to newsletter recipients.
    """
    import html as _html

    try:
        from_email, _ = _get_from_address()
    except EnvironmentError as exc:
        logger.error("Admin notice not sent — sender config error: %s", exc)
        return SendResult(success=False, error=str(exc))

    html_body = (
        '<div style="font-family:Arial,Helvetica,sans-serif;font-size:15px;'
        'line-height:1.5;white-space:pre-wrap;color:#111">'
        + _html.escape(text_body)
        + "</div>"
    )
    return send_email(
        to_email=from_email,
